...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const r = await query(`SELECT airport_distance_id FROM airport_distance_evidence WHERE id = $1`, [id]);
  if (!r.rows.length) return NextResponse.json({ data: null, error: '找不到佐證檔案' }, { status: 404 });
  const { airport_distance_id } = r.rows[0];

  const match = fs.existsSync(STORAGE_DIR)
    ? fs.readdirSync(STORAGE_DIR).find((f) => f.startsWith(airport_distance_id))
    : undefined;
