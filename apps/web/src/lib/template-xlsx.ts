...
export function buildTemplateWorkbook(sourceCode: string, year: string, nameZh: string, unit: string): Buffer {
  ...
  const ref = isSepticTank ? '上班總時數*' : refLabel(sourceCode);
  const qtyLabel = isSepticTank ? '上班天數*' : '用量*';
  const unitLabel = isSepticTank ? '上班人數' : '單位';
  const header = ['月份*', '排放源代碼*', '單據號碼', '單據日期', qtyLabel, unitLabel, ref, '備註', '公檔連結'];
  const example = isSepticTank
    ? [
        [6, sourceCode, '', '', 26, 120, 208, '第一次', ''],
        [6, sourceCode, '', '', 22, 118, 176, '', ''],
      ]
    : [
  ...
  // 說明分頁
  const note = isSepticTank
    ? XLSX.utils.aoa_to_sheet([
        ['單據明細匯入範本 — 說明'],
        [''],
        [`排放源：${nameZh || '(未指定)'}（代碼 ${sourceCode}）`],
        [''],
        ['1. 化糞池排放採「上班天數 × 上班人數 × 上班總時數」計算，非一般用量/單位模式。'],
        ['2. 欄名有 * 者為必填欄位：月份、上班天數、上班總時數；上班人數建議填寫（用於平均人數計算）。'],
        ['3. ⚠ 本範本僅供離線填寫參考，目前系統匯入功能尚未支援此排放源的自動上傳，'],
        ['   請將本範本內容改為手動輸入「填報頁」化糞池分頁對應月份的上班天數/人數/總時數欄位。'],
        ['4. 範例列可刪除或覆蓋。'],
      ])
    : XLSX.utils.aoa_to_sheet([
        ['單據明細匯入範本 — 說明'],
