...
export function buildTemplateWorkbook(sourceCode: string, year: string, nameZh: string, unit: string): Buffer {
  ...
  if (isTravel) {
    // 商務旅行專用格式：每一列 = 一趟出差（可多人同行），比照滅火器「+新增記錄」模式，
    // 系統匯入後各自新增一筆填報紀錄（不依月份合併），CO2e 依人數等比例放大，再彙總出
    // 各交通工具的人數/PKM(人公里)/CO2e。「交通工具」欄要填「飛機」「高鐵」或「火車」
    // （三選一，系統靠這欄文字判斷套用哪一種排放係數），拼錯或空白該列會被略過不會誤算。
    // 支援轉機/轉車：距離1 + 距離2（第二段）加總為總距離；距離1 二擇一，飛機常用海浬、
    // 系統自動換算成 km，「總距離(km)」欄僅供自行核對，系統一律重新加總不採信該欄數字。
    const header = [
      '日期*', '姓名或ID', '交通工具*(飛機/高鐵/火車)', '人數', '出發地', '中轉站', '目的地',
      '趟次(單程/來回)', '距離1(海浬)', '距離1(km)', '距離2(km，轉機/轉車)', '總距離(km，僅供參考)',
    ];
    const example = [
      ['2026-03-05', '王小明、李小美、陳大同', '飛機', 3, 'TPE', 'HKG', 'HAN', '來回', 500, '', 250, 1176],
      ['2026-03-12', '陳小華', '高鐵', 1, '台北', '', '台中', '單程', '', 150, '', 150],
    ];
    const ws = XLSX.utils.aoa_to_sheet([header, ...example]);
    ws['!cols'] = [
      { wch: 12 }, { wch: 20 }, { wch: 20 }, { wch: 8 }, { wch: 10 }, { wch: 10 }, { wch: 10 },
    ...
    const note = XLSX.utils.aoa_to_sheet([
      ['商務旅行匯入範本 — 說明'],
      [''],
      ['1. 每一列代表一趟出差，可多人同行（填「人數」欄，CO₂e 會依人數等比例放大）。'],
      ['   距離可以留空——系統會用出發地/目的地代碼查機場距離資料庫自動帶入；查不到才需要你填。'],
      ['2. 「交通工具」請填「飛機」「高鐵」或「火車」三選一的中文字，系統依此判斷套用哪一種排放係數；'],
      ['   拼錯字或留空的列會被系統略過，不會誤算或誤套係數。'],
      ['3. 「距離1」二擇一：飛機通常用「距離1(海浬)」，系統會自動換算成公里（1 海浬 = 1.852 公里）；'],
      ['   高鐵/火車直接填「距離1(km)」。兩欄都填時系統優先採用海浬換算的結果。'],
      ['4. 有轉機/轉車時，第二段距離填「距離2(km)」，系統會自動加總距離1+距離2得總距離；'],
      ['   「總距離(km，僅供參考)」欄只是方便你自己核對用的公式，系統計算時不會採用這欄的數字，'],
      ['   一律以距離1+距離2重新加總，避免公式沒更新或手動改錯數字而算出錯誤結果。'],
      ['5. 距離請填單程距離；「趟次」填「來回」時系統計算 CO₂e 會自動乘 2，不需自行加倍。'],
      ['6. 匯入後每一列會各自新增一筆填報紀錄（不依月份合併成一筆），填報頁「商務旅行 3.6」'],
      ['   分頁上方會顯示各交通工具的人數/PKM(人公里)/CO₂e 彙總表。'],
      ['7. 出發地/目的地請盡量填機場代碼（例如 TPE、HKG），距離1若留空系統會用這兩個代碼'],
      ['   查機場距離資料庫自動帶入；查不到的話該列還是會匯入，只是距離留白、算不出碳排，'],
      ['   填報頁「商務旅行」分頁會用黃底提示「缺距離」，就地補上距離＋佐證截圖即可，'],
      ['   不用抓來這份Excel重填——補過一次以後，同樣的直飛路線下次匯入就查得到了。'],
      ['8. 範例列可刪除或覆蓋。'],
    ]);
...
export function templateFilename(sourceCode: string, nameZh: string, year: string): string {
  // 商務旅行三種排放源（飛機/高鐵/火車）共用同一份範本內容，檔名統一叫「商務旅行」，
  // 不用照 emission_sources.name_zh 顯示「商務旅行-飛機出差」這種只反映其中一種交通工具的名稱。
  if (sourceCode.startsWith('3-6-') && sourceCode !== '3-6-B') {
    return `商務旅行匯入範本_${year}.xlsx`;
  }
