...
> - [`docs/decisions/`](docs/decisions/README.md) — 為什麼這樣做（決策紀錄），不是做了什麼
> - [`docs/待辦與已知缺口.md`](docs/待辦與已知缺口.md) — 累積中的未完成項目與風險
>
...
### 9. 動到資料庫結構、或改變既有規則時，補一筆決策紀錄
不是每個 commit 都要寫，但符合以下任一項就在 `docs/decisions/` 新增一筆
（複製 `_template.md`，格式：背景／決定／為什麼／現在還沒做的）：
- 新的 `V<n>__*.sql`
- 改變一個既有規則（例如停用改成刪除、權限模型改了）
- 做了取捨、砍了範圍，或刻意不做某件看起來該做的事
- Elodie 拍板了一個有截止日期或有爭議的決定

同時檢查 `docs/待辦與已知缺口.md`：這次改動有沒有解決清單裡的項目（移到「已解決」），
或又製造了新的已知缺口（加進「未解決」）。這份清單存在的目的，是讓下一個接手的人
（或下一個 Claude session）不必重新問一輪就知道現況邊界在哪——**不要讓缺口只留在
對話記憶或 memory 檔裡，那些下一個人看不到。**
